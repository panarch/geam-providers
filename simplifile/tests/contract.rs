...
#[test]
fn original_simplifile_and_filepath_run_through_public_host_boundary() {
    let temp = tempfile::tempdir().expect("isolated embedding file tree");
    let root = temp.path().join("contract");
    let root = root.to_str().expect("temporary path is Unicode");
    ...
    let program = project.compile().expect("original Hex packages compile");
